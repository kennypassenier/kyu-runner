# Debugging guide — kyu-runner

The evidence trail: what the runner tells you, what it means, and
where to look next. Written in Phase 8 from the code as built.

## Where to look

1. **The journal:** `journalctl -u kyu-runner -f` (systemd), or the
   process stderr. Default format is plain text; `KYU_RUNNER_LOG_FORMAT=json`
   gives Loki-ready lines. `KYU_RUNNER_LOG=info,kyu_runner=debug`
...
2. **The hub dashboard** (`http://10.10.10.9:8080/`): per topic, the
   ...
   runner does — the runner deliberately holds none.
...
|---|---|---|---|
| `kyu-runner started` | info | config valid, routes spawned | — |
...
|---|---|
| Nothing arrives in HA, no errors anywhere | (1) The HA automation does not exist or listens to another `webhook_id` — **HA answers 200 regardless**, so the runner acks into the void: run the runbook §1.8 smoke test. (2) The route was added after traffic started: the subscription starts from now (runbook §5.2 replays history). (3) The producer publishes to a different topic name. |
| Everything arrives twice (occasionally) | At-least-once doing its job after a kill/blink between POST and ack. Structural duplicates: check for two routes or two runner instances on the same `{topic, subscription}` — the config refuses the first, not the second machine. |
