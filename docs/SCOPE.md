...
**consume** from the kyu hub. HA can already produce trivially
(`rest_command` → `POST /t/<topic>`), but nothing in HA long-polls;
the runner is the missing pump. It implements proposals **P1**
(called `hub-bridge` there — this project was renamed to kyu-runner on
2026-08-30) and **P8** (hub self-monitoring) of the Kyu
Integration Study (`~/ObsidianVault/Home Assistant/Documentation/
Kyu Integration Study.md`), both rated Essential by Kenny on
2026-08-28.

The kyu interface authority is `~/Projects/kyu/docs/USER_GUIDE.md`
(v1.0.x, the three-verb HTTP contract).

## Goals

- **G1 · The stateless pump.** A config file defines routes
  `{topic, subscription, ha_webhook_url}`. Per route the runner
  ...
  delivery.
- **G2 · P8 built in.** A shipped default route `kyu.events` → HA
  ...
  `kyu_sweeper_age_ms` → Grafana. The HA-side webhook automation is
...
- **NG2 · Payload-agnostic.** Bytes in, bytes out: no parsing, no
  validation, no transformation, no content-based routing (kyu N5
  inherited). The envelope v1 schema is a DRAFT owned by the
  pipeline-v2 project; schema changes there do not touch the runner.
- **NG3 · Never in a critical path.** Study §7 degradation doctrine:
  hub or runner down returns the house to its 2026-08-28 behaviour,
  never worse. The freezer chain keeps zero new dependencies. This
  sentence survives into the Phase 4 freeze.
- **B1 · No reverse direction** (decided at the gate): the runner does
...
- **S2** HA down → the backlog accumulates on the hub; on HA's return
  the runner drains it **in publish order**, nothing lost.
- **S3** `kill -9` on the runner at any moment — including between the
...
- **R4 · Own app token** from the hub's `/apps` page (name:
  `kyu-runner`), independently revocable, never in git. How the token
  reaches the runner is the Phase 2 ecosystem item (latch is the
  ecosystem candidate).
...
| Option | Verdict |
|---|---|
| **Build `kyu-runner` (this project)** | **Chosen.** ~200 lines of logic; keeps all queue semantics (lease, retry, DLQ) working for HA delivery; ecosystem component. |
...
| Generic pipeline tools (Node-RED, Benthos/Redpanda Connect, n8n) | Rejected: none speak kyu's lease/ack/nack protocol natively; ack-on-200 semantics would need custom code inside a much larger runtime than the thing being built. |
| No consuming (HA produce-only) | Legitimate phase-1 posture, loses half the value; superseded by this project. |

Ecosystem findings (Phase 1): consumes **kyu** (the point of the
...
runs native under systemd on LXC 109 and the runner follows that
